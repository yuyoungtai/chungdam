...
const saveContList = async () => {
    ...
    if(checkList.length > 0){
        ...
        try{
            await axios.post('/updateCorpContList', DATA_LIST)
                .then(response => {
                    alert('계약항목 저장 완료!');
...
const cancelContList = async () => {
    ...
    if(cancelList.length > 0){
        ...
        try{
            await axios.post('/cancelCorpContList', DATA_LIST)
                .then(response=>{
                    alert('취소 처리 완료!');
                    //search.js
                    getContractList(document.querySelector('#current-corp-id').value);
                })
        }catch (e) {
            alert('계약항목 삭제 오류:'+e);
        }
